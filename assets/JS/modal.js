...
export function eventModal(){
    ...
    function initialiserContenuImage(){
     ...
     const envoyerDonneesAPI = async (formData) => {
    ...
    inputTitre.addEventListener('input', (event)=>{
        ...
        if(inputFile.files.length>0 && titreValue!=='' && selectCtegorie.value!=='0'){
            boutonValider.style.background='#1D6154'
            champsVide.style.display ='none'
    ...
    selectCtegorie.addEventListener('change',(event)=>{
        const categorie = event.target.value
        if(inputFile.files.length>0 && inputTitre.value!=='' && categorie!=='0'){
            boutonValider.style.background='#1D6154'
            champsVide.style.display ='none'
